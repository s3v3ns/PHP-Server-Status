...
{
    if (!$funName || trim($funName) == '' || preg_match('~[^a-z0-9\_]+~i', $funName, $tmp)) return 'й”™иЇЇ';
    return (false !== function_exists($funName)) ? 'ON' : 'OFF';
}
...
//е€¤ж–­е†…е­е¦‚жћње°ЏдєЋ1GпјЊе°±жѕз¤єMпјЊеђ¦е€™жѕз¤єGеЌ•дЅЌ
if($sysInfo['memTotal']<1024)
{
    ...
    $mc = $sysInfo['memCached']." M";	//cacheеЊ–е†…е­
    $mb = $sysInfo['memBuffers']." M";	//зј“е†І
    $st = $sysInfo['swapTotal']." M";
    $su = $sysInfo['swapUsed']." M";
    $sf = $sysInfo['swapFree']." M";
    $swapPercent = $sysInfo['swapPercent'];
    $memRealUsed = $sysInfo['memRealUsed']." M"; //зњџе®ће†…е­дЅїз”Ё
    $memRealFree = $sysInfo['memRealFree']." M"; //зњџе®ће†…е­з©єй—І
    $memRealPercent = $sysInfo['memRealPercent']; //зњџе®ће†…е­дЅїз”ЁжЇ”зЋ‡
    $memPercent = $sysInfo['memPercent']; //е†…е­жЂ»дЅїз”ЁзЋ‡
    $memCachedPercent = $sysInfo['memCachedPercent']; //cacheе†…е­дЅїз”ЁзЋ‡
}
else
{
    ...
    $swapPercent = $sysInfo['swapPercent'];
    $memRealUsed = round($sysInfo['memRealUsed']/1024,3)." G"; //зњџе®ће†…е­дЅїз”Ё
    $memRealFree = round($sysInfo['memRealFree']/1024,3)." G"; //зњџе®ће†…е­з©єй—І
    $memRealPercent = $sysInfo['memRealPercent']; //зњџе®ће†…е­дЅїз”ЁжЇ”зЋ‡
    $memPercent = $sysInfo['memPercent']; //е†…е­жЂ»дЅїз”ЁзЋ‡
    $memCachedPercent = $sysInfo['memCachedPercent']; //cacheе†…е­дЅїз”ЁзЋ‡
}
    ...
    <?if("show"==$sysReShow){?>
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            ...
            <tr>
                <td>Space usage</td>
                <td colspan="5">
                    Total Space <?php echo $dt;?>&nbsp;GпјЊ
                    Used <font color='#333333'><span id="useSpace"><?php echo $du;?></span></font>&nbsp;GпјЊ
                    Free <font color='#333333'><span id="freeSpace"><?php echo $df;?></span></font>&nbsp;GпјЊ
            ...
            <tr>
                ...
                <td colspan="5">
                    ...
                    <?php
                    //е€¤ж–­е¦‚жћњcacheдёє0пјЊдёЌжѕз¤є
                    ...
                    //е€¤ж–­е¦‚жћњSWAPеЊєдёє0пјЊдёЌжѕз¤є
...
?></span>
    ...
    <table>
        ...
        <tr>
            ...
            <td width="18%">
                ...
                ?>
                <?php
                echo (false!==preg_match("phpinfo", $disFuns))? '<span style="color: red">OFF</span>' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?>
    ...
    <table>
        ...
        <tr>
            ...
            <td width="18%"><?php $zend_version = zend_version();if(empty($zend_version)){echo '<font color=red>OFF</font>';}else{echo $zend_version;}?></td>
            <td width="32%">
                <?php
            ...
            <td width="18%"><?php if($PHP_VERSION > 2){echo (get_cfg_var("zend_loader.enable"))?'<font color=green>ON</font>':'<font color=red>OFF</font>';} else{if(function_exists('zend_optimizer_version')){	echo zend_optimizer_version();}else{	echo (get_cfg_var("zend_optimizer.optimization_level")||get_cfg_var("zend_extension_manager.optimizer_ts")||get_cfg_var("zend.ze1_compatibility_mode")||get_cfg_var("zend_extension_ts"))?'<font color=green>ON</font>':'<font color=red>OFF</font>';}}?></td>
        </tr>
        <tr>
            <td>eAccelerator</td>
            <td><?php if((phpversion('eAccelerator'))!=''){echo phpversion('eAccelerator');}else{ echo "<font color=red>OFF</font>";} ?></td>
            <td>ioncube</td>
            <td><?php if(extension_loaded('ionCube Loader')){   $ys = ioncube_loader_iversion();   $gm = ".".(int)substr($ys,3,2);   echo ionCube_Loader_version().$gm;}else{echo "<font color=red>OFF</font>";}?></td>
        </tr>
        <tr>
            <td>XCache</td>
            <td><?php if((phpversion('XCache'))!=''){echo phpversion('XCache');}else{ echo "<font color=red>OFF</font>";} ?></td>
            <td>APC</td>
            <td><?php if((phpversion('APC'))!=''){echo phpversion('APC');}else{ echo "<font color=red>OFF</font>";} ?></td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            <td class="w_foot"><A HREF="http://ITBurgas.com" target="_blank"><?php echo $title.$version;?></A></td>
